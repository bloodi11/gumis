...
class math_parser
{
   ...
   public:
    ...
    vector <string> ConvertToONP(string exp);
      ...
      void PreParse(string exp);
      float Parse(vector<string> exp);
};
...
void math_parser::PreParse(string exp)
{
   ...
   for(int i = 0; i < symbols.size(); i++)
    ...
    if(X==-1&&Y==-1)
    {
        float ret=Parse(symbols);
        std::cout <<endl<< "WYNIK = " << ret << endl << endl;
    }
    else
    {
        if(X!=-1)
...
float math_parser::Parse(vector<string> symbols)
{
   vector <string> stack;
